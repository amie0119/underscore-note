...
(function() {
  ...
  // already done
  // 其实自己想要实现的话也不一定需要用到闭包，只不过感觉这里是为了方便的设计3个用途相似的 api 
  // 用一个函数通过传入不同的函数实现不同的功能
  // 创建 assigner 函数的内部函数
  ...
  var createAssigner = function(keysFunc, undefinedOnly) {
    ...
    return function(obj) {
      // obj 就是被赋值的对象
      // 这里的 arguments 应该是闭包函数即返回函数的参数
      var length = arguments.length;
      // 至少应该传入两个参数，因为至少需要一个赋值，一个被赋值的对象
      // 如果只有一个的话，就不需要有改变直接返回
      ...
      for (var index = 1; index < length; index++) {
        ...
        for (var i = 0; i < l; i++) {
          var key = keys[i];
          // 如果 undefinedOnly 为 false 或者没有传入 undefinedOnly 就是说可以覆盖前面出现过的相同 key 的值
          // obj[key] = source[key] 肯定可以执行
          // 即 _.extend 和 _.extendOwn 方法
          // ===================================
          // undefinedOnly 为 true 即 _defaults 方法
          // 或者本身 obj 就没有这个 key 的键值对
          // 可以保证只取首次出现的 key 值
          if (!undefinedOnly || obj[key] === void 0) obj[key] = source[key];
        }
      }
      // 返回已经继承后面兑现该参数属性的第一个参数对象
      return obj;
    };
  };
  ...
  var eq = function(a, b, aStack, bStack) {
    ...
    // 0 === -0 为 true，但 0 和 -0 是 unequal
    // 1 / 0 = Infinity 1 / -0 = -Infinity Infinity === -Infinity false

    // 一个疑问？
    // 在这一步已经可以判断出很多相等的情况了，比如两个 string，Number 什么情况下会继续往下走呢
    if (a === b) return a !== 0 || 1 / a === 1 / b;
    // A strict comparison is necessary because `null == undefined`.
    // a 和 b 有一个是 null 或 undefined
    // null === null 和 undefined === undefined 的确为 true，但感觉没有比较的必要了
    if (a == null || b == null) return a === b;
    // Unwrap any wrapped objects.
    // 如果 a 或 b 是 underscore OOP 的对象
    // 在构造函数中已经把原始对象赋给 _wrapped 属性了
    if (a instanceof _) a = a._wrapped;
    if (b instanceof _) b = b._wrapped;
    // Compare `[[Class]]` names.
    // 先判断类型，如果类型不相同的话肯定是 unequal
    ...
    switch (className) {
      ...
      case '[object Number]':
        ...
        // +a !== +a 来判断是否为 NaN
        // 如果 a、b 都是 NaN 返回true，有一个是 NaN 返回 false
        if (+a !== +a) return +b !== +b;
        // An `egal` comparison is performed for other numeric values.
        // 如果 a 为 0，是 0 的话 1 / +a === 1 / b 上面提过
      ...
      case '[object Boolean]':
        ...
        // 如果是 Date 或 Boolean 两个类型，其实比较的也是数值，用 + 转化成数字类型
        return +a === +b;
    }
    ...
    // 到这里的话其实说明 a 是数组 那么 a b 类型相等，b 也是数组
    ...
    // 第一次调用 eq() 函数，没有传入 aStack 和 bStack 参数，所以不会进入 while
    // 其实相当于
    // while (length) {
    //   length--;
    // }
    while (length--) {
      ...
      // 这里还没想明白 如果 a 是数组 第二次进来不就 over 了？
    ...
    if (areArrays) {
      ...
      while (length--) {
        // 对展开的每一项，调用 eq() 比较，传入参数分别为 a, b 当前项, 各自比较过 stack
        // 只要有一项不相等了 return false
        if (!eq(a[length], b[length], aStack, bStack)) return false;
      }
    } else {
      // Deep compare objects.
      // 不是数组的话就是对象了，也要展开深度比较
      var keys = _.keys(a), key;
      length = keys.length;
      // Ensure that both objects contain the same number of properties before comparing deep equality.
      // 仍然是先比较长度~
      // 不过讲起来比较长度也是要遍历一遍，先比较长度感觉能省下很多比较的次数
    ...
    // 走到这里的话，证明当前这次比较的 a b isEqual
    // 当前比较的 a b 就在栈顶，pop 之
    aStack.pop();
    bStack.pop();
    return true;
  };
